package com.example.addon.chesttracker.impl.gui.screen;
...
import com.example.addon.chesttracker.api.memory.counting.CountingPredicate;
import com.example.addon.chesttracker.api.providers.ProviderUtils;
import com.example.addon.chesttracker.impl.compat.Compatibility;
import com.example.addon.chesttracker.impl.compat.mods.searchables.SearchablesUtil;
...
import com.example.addon.chesttracker.impl.gui.GuiConstants;
import com.example.addon.chesttracker.impl.gui.util.DevilsThemeRenderer;
import com.example.addon.chesttracker.impl.gui.util.TextColours;
import com.example.addon.chesttracker.impl.gui.widget.*;
...
import com.example.addon.chesttracker.impl.util.ItemStacks;
...
public class ChestTrackerScreen extends Screen {
    ...
    public void render(@NotNull GuiGraphics graphics, int mouseX, int mouseY, float tickDelta) {
        ...
        DevilsThemeRenderer.renderForegroundVeil(
                graphics,
                left + GRID_LEFT - 1,
                top + GRID_TOP - 1,
                this.itemList.getWidth() + 2,
                this.itemList.getHeight() + 2
        );
        graphics.drawString(this.font, this.title, left + TITLE_LEFT, top + TITLE_TOP, DevilsThemeRenderer.titleColor(TextColours.getLabelColour()), false); // titlebase
    ...
    public void renderBackground(@NotNull GuiGraphics graphics, int i, int j, float f) {
        ...
        DevilsThemeRenderer.renderPanelOverlay(graphics, left, top, menuWidth, menuHeight);
        DevilsThemeRenderer.renderSectionOverlay(graphics, left + SEARCH_LEFT - 2, top + SEARCH_TOP - 2, menuWidth - 12, 16);
        DevilsThemeRenderer.renderSectionOverlay(
                graphics,
                left + GRID_LEFT - 1,
                top + GRID_TOP - 1,
                this.itemList == null ? 0 : this.itemList.getWidth() + 2,
                this.itemList == null ? 0 : this.itemList.getHeight() + 2
        );
        DevilsThemeRenderer.renderSectionOverlay(graphics, left + menuWidth - 20, top + GRID_TOP - 1, 13, this.itemList == null ? 0 : this.itemList.getHeight() + 2);
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        ...
        if (!searchFocused && com.example.addon.chesttracker.impl.ChestTracker.OPEN_GUI.matches(keyCode, scanCode)) {
            this.onClose();
            return true;
        }
