...
public interface PositionUtils {
    ...
    static @Nullable RecipeBookComponent<?> getVisibleRecipe(AbstractContainerScreen<?> screen) {
        if (screen instanceof AbstractRecipeBookScreenAccessor recipeHolder && recipeHolder.devilsct$getRecipeBookComponent().isVisible()) {
            return recipeHolder.devilsct$getRecipeBookComponent();
        } else {
            return null;
        }
    }
    ...
    static Optional<ButtonPosition> calculate(AbstractContainerScreen<?> screen, int mouseX, int mouseY) {
        final int width = ((CTButtonScreenDuck) screen).devilsct$getWidth();
        final int recipeWidth = getRecipeComponentWidth(screen);
        final int height = ((CTButtonScreenDuck) screen).devilsct$getHeight();

        final int left = ((CTButtonScreenDuck) screen).devilsct$getLeft();
        final int top = ((CTButtonScreenDuck) screen).devilsct$getTop();
