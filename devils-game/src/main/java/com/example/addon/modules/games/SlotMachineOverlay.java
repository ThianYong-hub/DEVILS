...
public final class SlotMachineOverlay extends Module {
    ...
    private final SlotMachineWindow window = new SlotMachineWindow(MIN_W, MIN_H, MAX_W, MAX_H);
    private boolean windowInitialized;

    public SlotMachineOverlay() {
        super(DevilsGameAddon.GAMES_CATEGORY, "slot-machine", "Standalone slot machine launcher.");
        runInMainMenu = true;
    }
    ...
    public void onActivate() {
        ...
        GameLaunchCoordinator.activateExclusive(SlotMachineOverlay.class);
    ...
    private void onTick(TickEvent.Post event) {
        GameCrashGuard.run(this, "slotMachineTick", () -> {
            if (!isActive()) return;
            window.onTick();
    ...
    private void onRender2D(Render2DEvent event) {
        GameCrashGuard.run(this, "slotMachineRender", () -> {
            if (!isActive()) return;
    ...
    private void onMouse(MouseClickEvent event) {
        GameCrashGuard.run(this, "slotMachineMouse", () -> {
            if (!isActive()) return;
            if (mc.currentScreen instanceof WidgetScreen) return;
            boolean consumed = window.onMouse(
                ...
                pinned.get(),
                this::setPinned,
                () -> GameLaunchCoordinator.launchNext(GameLaunchCoordinator.Entry.SLOT_MACHINE),
                GameLaunchCoordinator::closeAll
            );
