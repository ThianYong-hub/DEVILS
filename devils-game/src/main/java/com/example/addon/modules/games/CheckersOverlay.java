...
public final class CheckersOverlay extends Module {
    ...
    private final Setting<PlayMode> mode = sgGeneral.add(new EnumSetting.Builder<PlayMode>()
        .name("mode")
        .description("Play against script or in Game Sync session.")
    ...
    private final CheckersOverlaySession session = new CheckersOverlaySession();
    private final CheckersOverlayWindow window = new CheckersOverlayWindow(MIN_W, MIN_H, MAX_W, MAX_H);
    private boolean windowInitialized;

    public CheckersOverlay() {
        super(DevilsGameAddon.GAMES_CATEGORY, "checkers", "Standalone checkers launcher with script and game-sync modes.");
        runInMainMenu = true;
    }
    ...
    public void onActivate() {
        ...
        GameLaunchCoordinator.activateExclusive(CheckersOverlay.class);
    ...
    private void onTick(TickEvent.Post event) {
        GameCrashGuard.run(this, "checkersOverlayTick", () -> {
            if (!isActive()) return;
            session.setScriptLevel(scriptLevel.get());
    ...
    private void onRender2D(Render2DEvent event) {
        GameCrashGuard.run(this, "checkersOverlayRender", () -> {
            if (!isActive()) return;
            window.render(
                event.drawContext,
                client(),
    ...
    private void onMouse(MouseClickEvent event) {
        GameCrashGuard.run(this, "checkersOverlayMouse", () -> {
            if (!isActive()) return;
            if (mc.currentScreen instanceof WidgetScreen) return;
            boolean consumed = window.onMouse(
                ...
                this::setMode,
                this::setScriptLevel,
                () -> GameLaunchCoordinator.launchNext(GameLaunchCoordinator.Entry.CHECKERS),
                GameLaunchCoordinator::closeAll,
                session
            );
