...
final class CheckersOverlaySession {
    ...
    void onActivate(CheckersOverlay.PlayMode mode) {
        stagedPath.clear();
        statusText = "";
        animation = null;
    ...
    void onTick(CheckersOverlay.PlayMode mode) {
        if (mode == CheckersOverlay.PlayMode.SYNC) runtime.tick();
        if (animation != null && System.currentTimeMillis() - animation.startedAtMs() >= animation.durationMs()) {
            animation = null;
        }
        if (mode == CheckersOverlay.PlayMode.SCRIPT) processPendingBotMove();
    }

    SessionView prepare(CheckersOverlay.PlayMode mode) {
        if (mode != CheckersOverlay.PlayMode.SYNC) return null;
    ...
    void toggleViewSide(CheckersOverlay.PlayMode mode) {
        if (mode == CheckersOverlay.PlayMode.SCRIPT) {
    ...
    boolean resolveOrientation(CheckersOverlay.PlayMode mode, SessionView session) {
        boolean whiteBottom = mode == CheckersOverlay.PlayMode.SYNC
    ...
    void onBoardClick(CheckersOverlay.PlayMode mode, int x, int y, SessionView session) {
        if (animation != null) return;

        boolean localWhite;
        if (mode == CheckersOverlay.PlayMode.SYNC) {
    ...
    private void applySyncSnapshot(SessionView session) {
        ...
        if (session == null || !session.active()) {
            statusText = "No active checkers session in Game Sync.";
            stagedPath.clear();
            return;
        }
