...
public final class XaeroTrackedPingRenderCache {
    ...
    public static synchronized void update(Collection<com.devils.addon.util.XaeroSyncWaypoints.PlayerMarker> trackedMarkers) {
        trackedPingRenderMarkers.clear();
        if (trackedMarkers == null) return;

        for (com.devils.addon.util.XaeroSyncWaypoints.PlayerMarker marker : trackedMarkers) {
    ...
    private static String buildPingLabelByMode(String sender, String coords, com.devils.addon.modules.Ping.InfoMode mode) {
        com.devils.addon.modules.Ping.InfoMode effectiveMode = mode == null ? com.devils.addon.modules.Ping.InfoMode.Distance : mode;
    ...
    private static com.devils.addon.modules.Ping.InfoMode resolvePingInfoMode() {
        try {
            var modules = meteordevelopment.meteorclient.systems.modules.Modules.get();
            if (modules == null) return com.devils.addon.modules.Ping.InfoMode.Distance;
            com.devils.addon.modules.Ping ping = modules.get(com.devils.addon.modules.Ping.class);
            if (ping == null) return com.devils.addon.modules.Ping.InfoMode.Distance;
            com.devils.addon.modules.Ping.InfoMode mode = ping.xaeroInfoMode();
            return mode == null ? com.devils.addon.modules.Ping.InfoMode.Distance : mode;
        } catch (Throwable ignored) {
            return com.devils.addon.modules.Ping.InfoMode.Distance;
        }
    }
}
