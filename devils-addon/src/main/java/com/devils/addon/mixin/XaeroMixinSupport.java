...
abstract class ModulesScreenIconsMixin {
    ...
    private static Texture devils$getIcon() {
        ...
        devilsIcon = devils$loadIcon("devils-addon", "category_icon.png", "Devils", () -> devilsLoadFailed = true);
        return devilsIcon;
    }
    ...
    private static Texture devils$getGameIcon() {
        ...
        devilsGameIcon = devils$loadIcon("devils-game", "icon.png", "Devils-Game", () -> devilsGameLoadFailed = true);
        return devilsGameIcon;
    }

    @Unique
    private static Texture devils$loadIcon(String namespace, String fileName, String label, Runnable onFail) {
        try {
            Identifier id = Identifier.of(namespace, fileName);
            var resource = MinecraftClient.getInstance().getResourceManager().getResource(id);
            if (resource.isEmpty()) {
                DevilsAddon.LOG.error("[Devils] {} icon '{}' not found in resources", label, fileName);
                onFail.run();
                return null;
            }
            ...
            try (MemoryStack stack = MemoryStack.stackPush()) {
                ...
                if (pixels == null) {
                    DevilsAddon.LOG.error("[Devils] {} icon decode failed: {}", label, STBImage.stbi_failure_reason());
                    onFail.run();
                    return null;
                }
                ...
                DevilsAddon.LOG.info("[Devils] {} icon loaded ({}x{})", label, texture.getWidth(), texture.getHeight());
                return texture;
            }
        } catch (Exception e) {
            DevilsAddon.LOG.error("[Devils] Failed to load {} icon '{}'", label, fileName, e);
            onFail.run();
            return null;
        }
    }
