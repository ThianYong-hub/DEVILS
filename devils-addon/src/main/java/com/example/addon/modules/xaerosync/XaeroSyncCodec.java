...
import com.example.addon.shared.sync.SyncCrypto;
...
public final class XaeroSyncCodec {
    ...
    public SyncPullResult sendPullRequest(String baseUrl, String deviceId, String token, String signingKey, int timeoutSec, String encryptionKey, long knownRevision, int waitMs) throws Exception {
        ...
        payload.addProperty("module", XaeroSyncConstants.MODULE_NAMESPACE);

        HttpRequest request = buildSyncRequest(baseUrl, XaeroSyncConstants.SYNC_PULL_PATH, payload.toString(), token, signingKey, timeoutSec);
    ...
    public SyncPushResult sendPushRequest(
        ...
        payload.addProperty("module", XaeroSyncConstants.MODULE_NAMESPACE);
        payload.add("profiles", SyncCrypto.encryptProfiles(toJsonArray(snapshot), encryptionKey, XaeroSyncConstants.MODULE_NAMESPACE));

        HttpRequest request = buildSyncRequest(baseUrl, XaeroSyncConstants.SYNC_PUSH_PATH, payload.toString(), token, signingKey, timeoutSec);
        HttpResponse<String> response = httpClient.send(request, HttpResponse.BodyHandlers.ofString());
    ...
    public String computeFingerprint(List<SyncXaeroData> snapshot) {
        try {
            MessageDigest digest = MessageDigest.getInstance("SHA-256");
            List<String> rows = new ArrayList<>(snapshot.size());
            for (SyncXaeroData data : snapshot) {
                rows.add(XaeroSyncValueUtils.normalizeKey(data.username()) + "|" + XaeroSyncValueUtils.normalizeServerKey(data.server()) + "|" + XaeroSyncValueUtils.safe(data.payload()));
            }
    ...
    public String validateSyncBaseUrl(String baseUrl) {
        ...
        try {
            ...
            return SyncJsonUtils.formatSyncException("bad-base-url", e, XaeroSyncConstants.SYNC_ERROR_DETAIL_MAX);
        }
    }
    ...
    private URI buildSyncStreamUri(String baseUrl, String deviceId, long knownRevision, int waitMs, boolean useLegacyPath) {
        String query =
            "deviceId=" + java.net.URLEncoder.encode(deviceId == null ? "" : deviceId, StandardCharsets.UTF_8)
                + "&module=" + XaeroSyncConstants.MODULE_NAMESPACE
                + "&knownRevision=" + knownRevision
                + "&waitMs=" + Math.max(50, waitMs);
        String path = useLegacyPath ? XaeroSyncConstants.SYNC_STREAM_PATH_LEGACY : XaeroSyncConstants.SYNC_STREAM_PATH;
