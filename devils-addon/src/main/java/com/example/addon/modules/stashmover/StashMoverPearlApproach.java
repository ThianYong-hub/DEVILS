...
final class StashMoverPearlApproach {
    ...
    private static final double CURRENT_THROW_HORIZONTAL_SQ = 0.85 * 0.85;
    ...
    static Vec3d resolveTarget(Vec3d explicitTarget, BlockPos water, Vec3d chamber) {
        ...
        if (water == null) return chamber;
        return Vec3d.ofCenter(water);
    }
    ...
    static BlockPos resolveApproachGoal(MinecraftClient mc, BlockPos water, Vec3d target, Vec3d chamber) {
        ...
        for (int dx = -1; dx <= 1; dx++) {
            for (int dz = -1; dz <= 1; dz++) {
                ...
                BlockPos candidate = water.add(dx, 0, dz);
                ...
                double score = playerDistance + waterDistance * 0.25;
        ...
        if (bestStandable != null) return bestStandable;
        return water;
    }
