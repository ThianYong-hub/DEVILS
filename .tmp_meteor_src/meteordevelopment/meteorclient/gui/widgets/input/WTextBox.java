...
public abstract class WTextBox extends WWidget {
    ...
    public boolean onKeyPressed(int key, int mods) {
        ...
        boolean control = class_310.field_1703 ? mods == GLFW_MOD_SUPER : mods == GLFW_MOD_CONTROL;

        if (control && key == GLFW_KEY_C) {
            if (cursor != selectionStart || cursor != selectionEnd) {
                mc.field_1774.method_1455(text.substring(selectionStart, selectionEnd));
            }
            return true;
        }
        ...
        else if (mods == ((class_310.field_1703 ? GLFW_MOD_SUPER : GLFW_MOD_CONTROL) | GLFW_MOD_SHIFT) && key == GLFW_KEY_A) {
            resetSelection();
        }
    ...
    public boolean onKeyRepeated(int key, int mods) {
        ...
        boolean controlShift = mods == ((SystemUtils.IS_OS_WINDOWS ? GLFW_MOD_ALT : class_310.field_1703 ? GLFW_MOD_SUPER : GLFW_MOD_CONTROL) | GLFW_MOD_SHIFT);
        ...
        if (control && key == GLFW_KEY_V) {
            ...
            String preText = text;
            String clipboard = mc.field_1774.method_1460();
            int addedChars = 0;
            ...
            if (cursor > 0 && cursor == selectionStart && cursor == selectionEnd) {
                ...
                int count = (mods == (SystemUtils.IS_OS_WINDOWS ? GLFW_MOD_ALT : class_310.field_1703 ? GLFW_MOD_SUPER : GLFW_MOD_CONTROL))
                    ? cursor
            ...
            if (cursor > 0) {
                ...
                else if (mods == (SystemUtils.IS_OS_WINDOWS ? GLFW_MOD_ALT : class_310.field_1703 ? GLFW_MOD_SUPER : GLFW_MOD_CONTROL)) {
                    cursor = 0;
                    resetSelection();
                }
                else if (altShift) {
    ...
    private void cursorChanged() {
        ...
        textStart = class_3532.method_15350(textStart, 0, Math.max(textWidth() - maxTextWidth(), 0));
    ...
    public void set(String text) {
        ...
        cursor = class_3532.method_15340(cursor, 0, text.length());
        selectionStart = cursor;
        selectionEnd = cursor;
