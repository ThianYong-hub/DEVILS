...
public abstract class GuiTheme implements ISerializable<GuiTheme> {
    ...
    public class_2487 toTag() {
        ...
        for (String id : windowConfigs.keySet()) {
            configs.method_10566(id, windowConfigs.get(id).toTag());
        }
    ...
    public GuiTheme fromTag(class_2487 tag) {
        tag.method_10562("settings").ifPresent(settings::fromTag);

        tag.method_10562("windowConfigs").ifPresent(configs -> {
            for (String id : configs.method_10541()) {
                windowConfigs.put(id, new WindowConfig().fromTag(configs.method_10562(id).get()));
            }
        });
