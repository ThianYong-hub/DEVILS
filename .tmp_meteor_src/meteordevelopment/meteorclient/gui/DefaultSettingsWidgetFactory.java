...
public class DefaultSettingsWidgetFactory extends SettingsWidgetFactory {
    ...
    private void genericW(WTable table, GenericSetting<?> setting) {
        ...
        edit.action = () -> mc.method_1507(setting.get().createScreen(theme));
    ...
    private void colorW(WTable table, ColorSetting setting) {
        ...
        edit.action = () -> mc.method_1507(new ColorSettingScreen(theme, setting));
    ...
    private void blockW(WTable table, BlockSetting setting) {
        ...
        WItem item = list.add(theme.item(setting.get().method_8389().method_7854())).widget();

        WButton select = list.add(theme.button("Select")).widget();
        select.action = () -> {
            BlockSettingScreen screen = new BlockSettingScreen(theme, setting);
            screen.onClosed(() -> item.set(setting.get().method_8389().method_7854()));

            mc.method_1507(screen);
        };

        reset(table, setting, () -> item.set(setting.get().method_8389().method_7854()));
    }
    ...
    private void blockListW(WTable table, BlockListSetting setting) {
        selectW(table, setting, () -> mc.method_1507(new BlockListSettingScreen(theme, setting)));
    }
    ...
    private void itemW(WTable table, ItemSetting setting) {
        ...
        select.action = () -> {
            ...
            screen.onClosed(() -> item.set(setting.get().method_7854()));

            mc.method_1507(screen);
        };

        reset(table, setting, () -> item.set(setting.get().method_7854()));
    }

    private void itemListW(WTable table, ItemListSetting setting) {
        selectW(table, setting, () -> mc.method_1507(new ItemListSettingScreen(theme, setting)));
    }

    private void entityTypeListW(WTable table, EntityTypeListSetting setting) {
        selectW(table, setting, () -> mc.method_1507(new EntityTypeListSettingScreen(theme, setting)));
    }

    private void enchantmentListW(WTable table, EnchantmentListSetting setting) {
        selectW(table, setting, () -> mc.method_1507(new EnchantmentListSettingScreen(theme, setting)));
    }

    private void moduleListW(WTable table, ModuleListSetting setting) {
        selectW(table, setting, () -> mc.method_1507(new ModuleListSettingScreen(theme, setting)));
    }

    private void packetListW(WTable table, PacketListSetting setting) {
        selectW(table, setting, () -> mc.method_1507(new PacketBoolSettingScreen(theme, setting)));
    }

    private void particleTypeListW(WTable table, ParticleTypeListSetting setting) {
        selectW(table, setting, () -> mc.method_1507(new ParticleTypeListSettingScreen(theme, setting)));
    }

    private void soundEventListW(WTable table, SoundEventListSetting setting) {
        selectW(table, setting, () -> mc.method_1507(new SoundEventListSettingScreen(theme, setting)));
    }

    private void statusEffectAmplifierMapW(WTable table, StatusEffectAmplifierMapSetting setting) {
        selectW(table, setting, () -> mc.method_1507(new StatusEffectAmplifierMapSettingScreen(theme, setting)));
    }

    private void statusEffectListW(WTable table, StatusEffectListSetting setting) {
        selectW(table, setting, () -> mc.method_1507(new StatusEffectListSettingScreen(theme, setting)));
    }

    private void storageBlockListW(WTable table, StorageBlockListSetting setting) {
        selectW(table, setting, () -> mc.method_1507(new StorageBlockListSettingScreen(theme, setting)));
    }

    private void screenHandlerListW(WTable table, ScreenHandlerListSetting setting) {
        selectW(table, setting, () -> mc.method_1507(new ScreenHandlerSettingScreen(theme, setting)));
    }
    ...
    private void blockDataW(WTable table, BlockDataSetting<?> setting) {
        ...
        button.action = () -> mc.method_1507(new BlockDataSettingScreen<>(theme, setting));
    ...
    private void potionW(WTable table, PotionSetting setting) {
        WHorizontalList list = table.add(theme.horizontalList()).expandX().widget();
        WItemWithLabel item = list.add(theme.itemWithLabel(setting.get().potion, class_1074.method_4662(setting.get().potion.method_7909().method_7876()))).widget();
    ...
    private void colorListWFill(WTable t, ColorListSetting setting) {
        ...
        for (SettingColor color : setting.get()) {
            ...
            edit.action = () -> {
                ...
                mc.method_1507(new ColorSettingScreen(theme, set));
            };
