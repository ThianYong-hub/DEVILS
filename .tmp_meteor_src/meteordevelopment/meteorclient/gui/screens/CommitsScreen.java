...
public class CommitsScreen extends WindowScreen {
    ...
    private void populateHeader(String headerMessage) {
        ...
        if (website != null) l.add(theme.button("Website")).widget().action = () -> class_156.method_668().method_670(website);
    ...
    private void populateCommits() {
        ...
        if (commits.length > 0) {
            ...
            for (Commit commit : commits) {
                ...
                t.add(theme.label(getMessage(commit))).widget().action = () -> class_156.method_668().method_670(String.format("https://github.com/%s/commit/%s", addon.getRepo().getOwnerName(), commit.sha));
                t.row();
            }
        }
