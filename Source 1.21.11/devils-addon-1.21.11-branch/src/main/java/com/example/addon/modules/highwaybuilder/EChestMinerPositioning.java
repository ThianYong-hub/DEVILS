...
final class EChestMinerPositioning {
    ...
    void nudgeAwayFromActionPos() {
        ...
        Vec3d player = com.example.addon.util.EntityPositionCompat.pos(mc.player);
    ...
    BlockPos findStandPos(BlockPos placePos) {
        ...
        for (Direction dir : Direction.Type.HORIZONTAL) {
            ...
            double distance = com.example.addon.util.EntityPositionCompat.pos(mc.player).squaredDistanceTo(Vec3d.ofCenter(candidate));
    ...
    boolean ensureActionPosition() {
        ...
        if (selfBlocking) {
            ...
            if (miner.standPos != null) {
                Vec3d standTarget = getSafeStandPoint(miner.standPos, miner.actionPos);
                if (horizontalDistanceSq(com.example.addon.util.EntityPositionCompat.pos(mc.player), standTarget)
        ...
        if (canOperateFromCurrentPos(miner.actionPos)) {
            ...
            miner.lastEnsurePos = com.example.addon.util.EntityPositionCompat.pos(mc.player);
            return true;
        }
        ...
        Vec3d standTarget = getSafeStandPoint(miner.standPos, miner.actionPos);
        double distSq = horizontalDistanceSq(com.example.addon.util.EntityPositionCompat.pos(mc.player), standTarget);
        ...
        Vec3d currentPos = com.example.addon.util.EntityPositionCompat.pos(mc.player);
