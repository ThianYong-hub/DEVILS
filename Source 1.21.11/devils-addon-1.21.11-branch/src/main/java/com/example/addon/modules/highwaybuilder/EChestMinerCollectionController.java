...
final class EChestMinerCollectionController {
    ...
    int countGroundObsidian() {
        ...
        for (var entity : mc.world.getEntities()) {
            ...
            if (!searchBox.contains(com.example.addon.util.EntityPositionCompat.pos(itemEntity))) continue;
            if (!isInsideTrackedFarmArea(com.example.addon.util.EntityPositionCompat.pos(itemEntity))) continue;
    ...
    private ItemEntity findClosestObsidian() {
        ...
        for (var entity : mc.world.getEntities()) {
            ...
            if (!searchBox.contains(com.example.addon.util.EntityPositionCompat.pos(itemEntity))) continue;
            if (!isInsideTrackedFarmArea(com.example.addon.util.EntityPositionCompat.pos(itemEntity))) continue;
            double distance = com.example.addon.util.EntityPositionCompat.pos(itemEntity).squaredDistanceTo(com.example.addon.util.EntityPositionCompat.pos(mc.player));
        ...
        if (closest == null && !miner.farmCenters.isEmpty()) {
            ...
            for (var entity : mc.world.getEntities()) {
                if (!(entity instanceof ItemEntity itemEntity)) continue;
                if (itemEntity.getStack().getItem() != Items.OBSIDIAN) continue;
                if (!fallback.contains(com.example.addon.util.EntityPositionCompat.pos(itemEntity))) continue;
                double distance = com.example.addon.util.EntityPositionCompat.pos(itemEntity).squaredDistanceTo(playerPos);
                if (distance < closestDist) {
                    closestDist = distance;
                    closest = itemEntity;
                }
    ...
    private Box getObsidianSearchBox() {
        ...
        Vec3d center = miner.collectionCenter != null ? Vec3d.ofCenter(miner.collectionCenter) : com.example.addon.util.EntityPositionCompat.pos(mc.player);
