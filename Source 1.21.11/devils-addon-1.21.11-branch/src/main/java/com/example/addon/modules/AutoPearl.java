...
public class AutoPearl extends Module {
    ...
    public AutoPearl() {
        super(AddonTemplate.CATEGORY, "auto-pearl", "Locks onto a target and chases with ender pearls. Supports remote commands via chat (!pearl <nick>).");
    }
    ...
    private void onTickSafe(TickEvent.Pre event) {
        ...
        if (waitingForTeleport) {
            waitTicks++;
            if (posBeforeThrow != null && com.example.addon.util.EntityPositionCompat.pos(mc.player).distanceTo(posBeforeThrow) > TELEPORT_THRESHOLD) {
                waitingForTeleport = false;
                posBeforeThrow = null;
                waitTicks = 0;
                tickTimer = 0;
        ...
        prevTarget = target;
        prevTargetPos = com.example.addon.util.EntityPositionCompat.pos(target);
        ...
        posBeforeThrow = com.example.addon.util.EntityPositionCompat.pos(mc.player);
    ...
    private void onRender3DSafe(Render3DEvent event) {
        ...
        if (showRange.get()) {
            double r = range.get();
            Vec3d p = com.example.addon.util.EntityPositionCompat.pos(mc.player);
