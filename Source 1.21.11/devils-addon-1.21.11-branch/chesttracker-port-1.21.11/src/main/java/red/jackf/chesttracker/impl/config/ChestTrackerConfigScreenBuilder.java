package com.example.addon.chesttracker.impl.config;
...
import com.example.addon.chesttracker.impl.gui.invbutton.PositionExporter;
import com.example.addon.chesttracker.impl.memory.MemoryBankAccessImpl;
import com.example.addon.chesttracker.impl.storage.backend.Backend;
    ...
    private static OptionGroup makeGuiGroup(ConfigClassHandler<ChestTrackerConfig> instance) {
        return OptionGroup.createBuilder()
                ...
                .option(Option.<Boolean>createBuilder()
                        ...
                        .build())
                .option(Option.<Boolean>createBuilder()
                        .name(translatable("chesttracker.config.gui.devilsTheme"))
                        .description(OptionDescription.of(translatable("chesttracker.config.gui.devilsTheme.description")))
                        .controller(opt -> BooleanControllerBuilder.create(opt)
                                .yesNoFormatter()
                                .coloured(true))
                        .binding(
                                instance.defaults().gui.devilsTheme,
                                () -> instance.instance().gui.devilsTheme,
                                b -> {
                                    instance.instance().gui.devilsTheme = b;
                                    ChestTrackerRuntimeState.setDevilsThemeEnabled(b);
                                })
                        .build())
                .option(Option.<Integer>createBuilder()
                        .name(translatable("chesttracker.config.gui.devilsOverlayAlpha"))
                        .description(OptionDescription.of(translatable("chesttracker.config.gui.devilsOverlayAlpha.description")))
                        .controller(opt -> IntegerSliderControllerBuilder.create(opt)
                                .range(0, 255)
                                .step(1)
                                .formatValue(i -> translatable("chesttracker.config.gui.devilsOverlayAlpha.value", i)))
                        .binding(
                                instance.defaults().gui.devilsOverlayAlpha,
                                () -> instance.instance().gui.devilsOverlayAlpha,
                                i -> {
                                    instance.instance().gui.devilsOverlayAlpha = i;
                                    ChestTrackerRuntimeState.setDevilsOverlayAlpha(i);
                                })
                        .build())
                .build();
    }
    ...
    private static OptionGroup makeStorageGroup(ConfigClassHandler<ChestTrackerConfig> instance, Screen parent) {
        return OptionGroup.createBuilder()
                ...
                .option(ButtonOption.createBuilder()
                        ...
                        .build())
                .option(LabelOption.create(translatable("chesttracker.config.storage.devilsLockedNbt")))
                .option(Option.<Boolean>createBuilder()
