...
import com.example.addon.chesttracker.api.memory.counting.StackMergeMode;
...
public interface MemoryBank {
     ...
     */
    Map<Identifier, MemoryKey> getAllMemories();
     ...
     * @return A memory at the given key and position, or an empty optional if not present.
     */
    default Optional<Memory> getMemory(Identifier keyId, BlockPos position) {
     ...
     */
    default Optional<Memory> getMemory(Optional<Identifier> keyId, BlockPos position) {
        return getKey(keyId).flatMap(key -> key.get(position));
    }
     ...
     */
    Optional<MemoryKey> getKey(Identifier keyId);

    /**
     * Convenience overload for {@link #getKey(Identifier)} for faster use with {@link ProviderUtils#getPlayersCurrentKey()}.
     *
     * @param keyId Optional possibly containing a memory key to lookup.
     * @return An optional possibly containing a matching memory key.
     */
    default Optional<MemoryKey> getKey(Optional<Identifier> keyId) {
     ...
     */
    default List<ItemStack> getCounts(Identifier keyId, CountingPredicate predicate, StackMergeMode stackMergeMode) {
     ...
     * @return A list of stacks from all memories in the given key matching the predicate, merged according to <code>stackMergeMode</code>.
     */
    List<ItemStack> getCounts(Identifier keyId, CountingPredicate predicate, StackMergeMode stackMergeMode, boolean unpackNested);
     ...
     * memory key if empty.
     */
    void addMemory(Identifier keyId, BlockPos location, Memory memory);
     ...
     */
    void removeMemory(Identifier keyId, BlockPos location);
}
