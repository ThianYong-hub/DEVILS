...
import com.example.addon.chesttracker.impl.datafix.Types;
import com.example.addon.chesttracker.impl.memory.key.ManualMode;
import com.example.addon.chesttracker.impl.memory.key.OverrideInfo;
import com.example.addon.chesttracker.impl.memory.key.SearchContext;
...
import com.example.addon.chesttracker.impl.util.ModCodecs;
...
public class MemoryKeyImpl implements MemoryKey {
    ...
    public List<ItemStack> getCounts(CountingPredicate predicate, StackMergeMode stackMergeMode, boolean unpackNested) {
        var level = Minecraft.getInstance().level;
        boolean entitiesEnabled = com.example.addon.chesttracker.impl.config.ChestTrackerConfig.INSTANCE.instance().storage.entityMemories;
