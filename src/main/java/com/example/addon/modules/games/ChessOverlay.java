...
public final class ChessOverlay extends Module {
    ...
    private final Setting<PlayMode> mode = sgGeneral.add(new EnumSetting.Builder<PlayMode>()
        .name("mode")
        .description("Play against script or SyncHub.")
    ...
    public ChessOverlay() {
        super(AddonTemplate.GAMES_CATEGORY, "chess-overlay", "Floating chess window with script/sync modes.");
        runInMainMenu = true;
    }
    ...
    private void onTick(TickEvent.Post event) {
        CrashGuard.run(this, "chessOverlayTick", () -> {
            if (!isActive()) return;
            session.setScriptLevel(scriptLevel.get());
    ...
    private void onRender2D(Render2DEvent event) {
        CrashGuard.run(this, "chessOverlayRender", () -> {
            if (!isActive()) return;
    ...
    private void onMouse(MouseClickEvent event) {
        CrashGuard.run(this, "chessOverlayMouse", () -> {
            if (!isActive()) return;
            if (mc.currentScreen instanceof WidgetScreen) return;
            boolean consumed = window.onMouse(
                ...
                this::setMode,
                this::setScriptLevel,
                () -> GamesModule.cycleFrom(GamesModule.QuickGame.CHESS),
                GamesModule::closeAllFromAny,
                session
            );
