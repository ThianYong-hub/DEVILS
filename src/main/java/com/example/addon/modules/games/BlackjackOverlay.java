...
public final class BlackjackOverlay extends Module {
    ...
    public BlackjackOverlay() {
        super(AddonTemplate.GAMES_CATEGORY, "blackjack-overlay", "Blackjack mini-game in movable overlay.");
        runInMainMenu = true;
    }
    ...
    public void onActivate() {
        ...
        GamesModule.markActive(GamesModule.QuickGame.BLACKJACK);
    ...
    private void onTick(TickEvent.Post event) {
        CrashGuard.run(this, "blackjackTick", () -> {
            if (!isActive()) return;
            window.onTick();
    ...
    private void onRender2D(Render2DEvent event) {
        CrashGuard.run(this, "blackjackRender", () -> {
            if (!isActive()) return;
    ...
    private void onMouse(MouseClickEvent event) {
        CrashGuard.run(this, "blackjackMouse", () -> {
            if (!isActive()) return;
            if (mc.currentScreen instanceof WidgetScreen) return;
            boolean consumed = window.onMouse(
                ...
                pinned.get(),
                this::setPinned,
                () -> GamesModule.cycleFrom(GamesModule.QuickGame.BLACKJACK),
                GamesModule::closeAllFromAny
            );
