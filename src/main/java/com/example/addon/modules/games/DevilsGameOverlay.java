...
public final class DevilsGameOverlay extends Module {
    ...
    private final Setting<PlayMode> mode = sgGeneral.add(new EnumSetting.Builder<PlayMode>()
        .name("mode")
        .description("Play against script or in SyncHub session.")
    ...
    private final DevilsGameOverlaySession session = new DevilsGameOverlaySession();
    private final DevilsGameOverlayWindow window = new DevilsGameOverlayWindow(MIN_W, MIN_H, MAX_W, MAX_H);
    private boolean windowInitialized;

    public DevilsGameOverlay() {
        super(AddonTemplate.GAMES_CATEGORY, "devils-game-overlay", "Floating checkers window with drag, resize and pin.");
        runInMainMenu = true;
    }
    ...
    public void onActivate() {
        ...
        GamesModule.markActive(GamesModule.QuickGame.CHECKERS);
    ...
    private void onTick(TickEvent.Post event) {
        CrashGuard.run(this, "devilsGameOverlayTick", () -> {
            if (!isActive()) return;
            session.setScriptLevel(scriptLevel.get());
    ...
    private void onRender2D(Render2DEvent event) {
        CrashGuard.run(this, "devilsGameOverlayRender", () -> {
            if (!isActive()) return;
            window.render(
                event.drawContext,
                client(),
    ...
    private void onMouse(MouseClickEvent event) {
        CrashGuard.run(this, "devilsGameOverlayMouse", () -> {
            if (!isActive()) return;
            if (mc.currentScreen instanceof WidgetScreen) return;
            boolean consumed = window.onMouse(
                ...
                this::setMode,
                this::setScriptLevel,
                () -> GamesModule.cycleFrom(GamesModule.QuickGame.CHECKERS),
                GamesModule::closeAllFromAny,
                session
            );
