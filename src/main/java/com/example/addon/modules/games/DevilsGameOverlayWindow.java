...
final class DevilsGameOverlayWindow {
    ...
    private static final Identifier WHITE_PIECE = Identifier.of("devils-addon", "textures/games/checkers/piece_white.png");
    private static final Identifier BLACK_PIECE = Identifier.of("devils-addon", "textures/games/checkers/piece_black.png");
    ...
    DevilsGameOverlayWindow(int minW, int minH, int maxW, int maxH) {
        this.minW = minW;
        this.minH = minH;
        this.maxW = maxW;
        this.maxH = maxH;
    ...
    void render(
        DrawContext context,
        MinecraftClient mc,
        DevilsGameOverlay.PlayMode mode,
        boolean pinned,
        DevilsGameOverlaySession session,
        int scriptLevel
    ) {
        ...
        DevilsGameOverlaySession.AnimatedPiece animated = session.animatedPiece();
    ...
    boolean onMouse(
        ...
        Consumer<DevilsGameOverlay.PlayMode> setMode,
        ...
        if (inside(mouseX, mouseY, l.modeX, l.modeY, l.btnW, l.btnH)) {
            if (mode == DevilsGameOverlay.PlayMode.SCRIPT) {
                if (event.button() == 0) {
                    ...
                    setMode.accept(DevilsGameOverlay.PlayMode.SYNC);
                    session.onActivate(DevilsGameOverlay.PlayMode.SYNC);
                }
            } else {
                setMode.accept(DevilsGameOverlay.PlayMode.SCRIPT);
                session.onActivate(DevilsGameOverlay.PlayMode.SCRIPT);
            }
            return true;
        }
    ...
    private void drawHeader(
        ...
        String modeText = mode == DevilsGameOverlay.PlayMode.SCRIPT ? ("L" + scriptLevel) : "Sync";
    ...
    private void drawPieces(
        ...
        boolean whiteBottom,
        char[][] matrix,
        DevilsGameOverlaySession.AnimatedPiece animated,
        DevilsGameOverlaySession session
    ) {
    ...
    private void drawFooter(
        ...
        context.drawTextWithShadow(tr, "Mode: " + (mode == DevilsGameOverlay.PlayMode.SYNC ? "SyncHub" : "Script"), l.panelX + 8, l.panelY + 7, 0xFFE9F2FF);
        ...
        if (mode == DevilsGameOverlay.PlayMode.SYNC && session != null && session.active()) {
        ...
        } else if (mode == DevilsGameOverlay.PlayMode.SCRIPT) {
