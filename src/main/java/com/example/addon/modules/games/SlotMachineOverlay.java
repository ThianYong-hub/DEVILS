...
public final class SlotMachineOverlay extends Module {
    ...
    public SlotMachineOverlay() {
        super(AddonTemplate.GAMES_CATEGORY, "slot-machine", "One-armed bandit mini-game in movable overlay.");
        runInMainMenu = true;
    }
    ...
    private void onRender2D(Render2DEvent event) {
        CrashGuard.run(this, "slotMachineRender", () -> {
            if (!isActive()) return;
    ...
    private void onMouse(MouseClickEvent event) {
        CrashGuard.run(this, "slotMachineMouse", () -> {
            if (!isActive()) return;
            if (mc.currentScreen instanceof WidgetScreen) return;
            boolean consumed = window.onMouse(
                ...
                pinned.get(),
                this::setPinned,
                () -> GamesModule.cycleFrom(GamesModule.QuickGame.SLOT_MACHINE),
                GamesModule::closeAllFromAny
            );
