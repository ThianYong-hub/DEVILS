...
public final class DoomOverlay extends Module {
    ...
    public DoomOverlay() {
        super(AddonTemplate.GAMES_CATEGORY, "devilsdoom-overlay", "Embedded DevilsDoom window.");
        runInMainMenu = true;
    }
    ...
    public void onActivate() {
        ...
        GamesModule.markActive(GamesModule.QuickGame.DOOM);
        window.onActivate();
    ...
    private void onTickPre(TickEvent.Pre event) {
        CrashGuard.run(this, "devilsDoomTickPre", () -> {
            if (!isActive()) return;
    ...
    private void onTick(TickEvent.Post event) {
        CrashGuard.run(this, "devilsDoomTick", () -> {
            if (!isActive()) return;
            window.onTick();
    ...
    private void onKey(KeyEvent event) {
        CrashGuard.run(this, "devilsDoomKey", () -> {
            if (!isActive()) return;
            if (mc.currentScreen instanceof WidgetScreen) return;
            boolean consumed = window.onKey(event, client(), pinned.get(), GamesModule::closeAllFromAny);
    ...
    private void onRender2D(Render2DEvent event) {
        CrashGuard.run(this, "devilsDoomRender", () -> {
            if (!isActive()) return;
    ...
    private void onMouse(MouseClickEvent event) {
        CrashGuard.run(this, "devilsDoomMouse", () -> {
            if (!isActive()) return;
            if (mc.currentScreen instanceof WidgetScreen) return;
            boolean consumed = window.onMouse(
                ...
                pinned.get(),
                this::setPinned,
                () -> GamesModule.cycleFrom(GamesModule.QuickGame.DOOM),
