...
final class DoomSession {
    private static final String BUNDLED_BASE = "assets/devils-addon/games/doom/runtime/";
    ...
    void startIfNeeded() {
        ...
        try {
            ...
            AddonTemplate.LOG.warn("[DevilsDoom] Startup failed", t);
        } finally {
            starting = false;
        }
    }
    ...
    private void runLoop(Method setupLoop) {
        try {
            ...
            if (cause != null && "devilsdoom.Engine$DoomEmbeddedExit".equals(cause.getClass().getName())) {
                ...
                AddonTemplate.LOG.warn("[DevilsDoom] Runtime crashed", ite);
            }
        } catch (Throwable t) {
            statusText = "DevilsDoom crashed.";
            logText = shorten(unwrap(t).toString());
            AddonTemplate.LOG.warn("[DevilsDoom] Runtime crashed", t);
        } finally {
            running = false;
            starting = false;
    ...
    private static Path resolveRuntimeDir() {
        ...
        Path base = (mc != null && mc.runDirectory != null)
            ...
            : Paths.get(System.getProperty("user.home", "."), ".devils-addon");
    ...
    private void ensureTexture(MinecraftClient mc, int width, int height) {
        ...
        frameTextureId = Identifier.of("devils-addon", "dynamic/devilsdoom/frame_" + (++textureNonce));
