...
public final class PingSyncStreamController {
    ...
    public PingSyncStreamController(Ping module, PingSyncCodec codec, PingSyncProblemTracker problemTracker) {
        this.module = module;
        this.codec = codec;
        ...
        try {
            ...
            if (response.statusCode() < 200 || response.statusCode() >= 300) {
                ...
                if (response.statusCode() == 404 && syncStreamUseLegacyPath) {
                    ...
                    syncStreamUnsupportedUntilMs = System.currentTimeMillis() + PingConstants.SYNC_STREAM_UNSUPPORTED_BACKOFF_MS;
            ...
            module.client().execute(() -> {
                ...
                syncStreamReconnectAtMs = 0;
                module.logSyncInternal("Ping sync stream connected.");
            });
            ...
            if (!syncStreamStopRequested) {
                streamError = SyncJsonUtils.formatSyncException("stream-error", throwable, PingConstants.SYNC_ERROR_DETAIL_MAX);
            }
        } finally {
            ...
            module.client().execute(() -> {
                ...
                syncStreamFuture = null;
                if (!syncStreamStopRequested) {
                    long reconnectDelay = switch (problemTracker.classifySyncError(finalStreamError)) {
                        case AUTH -> PingConstants.SYNC_AUTH_BACKOFF_MS;
                        case CONFIG -> PingConstants.SYNC_CONFIG_BACKOFF_MS;
                        case CRYPTO -> PingConstants.SYNC_CRYPTO_BACKOFF_MS;
                        case NETWORK -> PingConstants.SYNC_NETWORK_BACKOFF_MS;
                        case OTHER -> PingConstants.SYNC_STREAM_RECONNECT_MS;
                    };
